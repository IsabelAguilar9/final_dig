...
void setup()
{
    ...
    xTaskCreatePinnedToCore(
        Task_bluetooth,"bluetooth_cuadro", 2048, NULL,1,NULL, 1);
    xTaskCreatePinnedToCore(
        Task_blink,"Secuencia de leds",2048, NULL, 1, &id_TaskBlink, 0);
    xTaskCreate(Task_read_keypad, "Task_read_keypad", 4096, NULL, 1, NULL);
    xTaskCreate(Task_process_Key, "Task_process_Key", 4096, NULL, 2, NULL);
    xTaskCreate(Task_rueda, "Task_rueda", 4096, NULL, 3, NULL);
}
            ...
            if (currentTime - lastTime >= movementSpeed / portTICK_PERIOD_MS)
            {
                ...
                if (GamePad.isDownPressed())
                {
                    ...
                    if (y > (SCREEN_HEIGHT - cuadGrande / 2 - 1))
                        y = SCREEN_HEIGHT - cuadGrande / 2 - 1; // Limitar el movimiento hacia abajo
                }
                if (GamePad.isUpPressed())
                {
                    y -= 4; // Mover hacia arriba si se presiona el botón "Up"
                    if (y < (cuadGrande / 2))
                        y = cuadGrande / 2; // Limitar el movimiento hacia arriba
                }
                ...
                if (GamePad.isRightPressed())
                {
                    ...
                    if (x > (56))
                        x = 56; // Limitar el movimiento hacia la derecha
                }
                display.drawRect(0, 0, 128, 64, SSD1306_WHITE);
                display.drawLine(64, 0, 64, 63, SSD1306_WHITE);
                // animacion del cuadro
