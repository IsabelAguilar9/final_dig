...
// Función para convertir la tecla presionada a milisegundos
int keyToMilliseconds(char key) {
  switch (key) {
    ...
    {
        Serial.begin(9600);
// crear cola para poder compartir eñ valor de la tecla
        keyQueue = xQueueCreate(10, sizeof(char));
        if (keyQueue == NULL)
        {
            Serial.println("Error creando la cola");
            while (true)
                ; // Si no se puede crear la cola, detener el programa
        }
...
void displayWheel(const unsigned char *bitmap)
{
    display.drawBitmap(
...
void Task_read_keypad(void *parameter)
{
    ...
    while (true)
    {
        ...
        vTaskDelay(10 / portTICK_PERIOD_MS); // Pequeño retraso para evitar rebotes
    }
}
void Task_process_Key(void *parameter) {
    char key;
    while (true)
    {
        ...
        {
            // Convertir la tecla a milisegundos
            int delayTime = keyToMilliseconds(key);
            if (delayTime != -1)
            {
                Serial.print("Tecla recibida para procesar: ");
                Serial.print(key);
                Serial.print(" - Delay en ms: ");
...
void Task_rueda(void *parameter)
{
    ...
    while (true)
    {
        displayWheel(wheelFrames[rueda]);
        rueda = (rueda + 1) % 4;
